...
export default async function handler(req, res) {
    ...
    // Pobieramy bezpiecznie klucz z ustawień Vercela
    ...
    // Odbieramy temat (prompt) od aplikacji (HTML)
    const { prompt } = req.body;

    try {
        // Wysyłamy zapytanie do Google z bezpiecznego miejsca
        ...
        // Zwracamy wygenerowany tekst do naszej aplikacji HTML
        res.status(200).json(data);
        
    } catch (error) {
        console.error("API Error:", error);
        res.status(500).json({ error: 'Błąd połączenia z Google AI' });
    }
}
